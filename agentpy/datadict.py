...
class DataDict(AttrDict):
    ...
    def calc_average_reporters(self, method='mean'):
        """ Calculates average reporters over multiple iterations
        and return self with a new entry 'average_reporters'.

        Arguments:
            method (str, optional):
                Method to use for average.
                Options are 'mean' (default), 'median', and 'mode'.
        """
        df = self.reporters
        if method == 'mean':
            ndf = df.groupby('sample_id').mean()
        elif method == 'median':
            ndf = df.groupby('sample_id').median()
        elif method == 'mode':
            ndf = df.groupby('sample_id').mode()
        else:
            raise AgentpyError("Method must be 'mean', 'median', or 'mode'.")
        self['average_reporters'] = ndf
        return self
    ...
    def calc_sobol(self, reporters=None, **kwargs):
        ...
        average reporters must first be calculated with
        :func:`DataDict.calc_average_reporters`.
        ...
        if not self.parameters.sample_log['type'] == 'saltelli':
            raise AgentpyError("Sampling method must be 'saltelli'.")
        if self.log['iterations'] == 1:
            df = self.reporters
        else:
            if 'average_reporters' in self:
                df = self.average_reporters
            else:
                raise AgentpyError(
                    "Average reporters must first be calculated with"
                    "DataDict.calc_average_reporters.")

        # STEP 1 - Load salib problem from parameter log
        param_ranges_salib = self.parameters.sample_log['salib_problem']
        calc_second_order = self.parameters.sample_log['calc_second_order']
